...
def evaluate(tile_actions, visualize=False):
    ...
    while True:
        ...
        steps += 1
        if terminated or truncated or restart or info["current_tile"] >= track_length - 1:
            break
        if info['wheels_on_track'] == 0:
            # total_reward += 100
            break
        if info['speed'] < 0.1 and a[1] == 0:
            # print(f"Stopped, Fitness: {total_reward}")
            total_reward += 100
            break
        if visualize:
            global quit
...
# fits, actions_best, x_history = oneplus_lambda(x0=tile_actions, fitness=f, gens=100)
# fits, actions_best, x_history = cma_es(x0=tile_actions, fitness=f, gens=100, popsize=50)
fits, actions_best, x_history = mutation(x0=tile_actions, fitness=f, gens=500)
# print(actions_best)
